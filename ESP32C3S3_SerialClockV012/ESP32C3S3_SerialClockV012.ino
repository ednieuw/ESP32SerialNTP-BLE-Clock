...
--------------------------------------  
 ...
 Changes V010: Updated web page
 Changes V011: Stable version
 Changes V012: Updated Serial1Check. Read buffer until empty and accept only valid chars

How to compile: Install ESP32 boards
ESP32C3_SUPERMINI
Elect Board: Nologo ESP32C3 Super mini
Compile and upload

ESP32S3_ZERO
Board: ESP32-S3 DEV module
Flash size 4MB
Partition Scheme: Default or change to Minimal SPIFFS (1.9Mb with OTA, 190kB SPIFFS) if there is a memoty shortage
...
#define TX1PIN 4 
                        ...
                        #ifdef NANOESP32 
#define RX1PIN A0 // 1 Connect to Rx on Word clock PCB
#define TX1PIN A1 // 2 Connect to Tx on Word clock PCB
                        #endif //NANOESP32
//--------------------------------------------                                                //
...
void SerialCheck(void)
{
 ...
 while (Serial.available())
    { 
     ...
     if (c>9 && c<128) SerialString += c;                                                    // Allow input from Space - Del
...
//--------------------------------------------
// Common check for serial input from Word clock
//--------------------------------------------
void Serial1Check(void)
{
 ...
 while (Serial1.available())
    ...
    if (c>9 && c<128) SerialString += c;                                                    // Allow input from Space - Del
     else c = 0;                                                                              // Delete a CR                                                                            // Delete a CR
    }
 if (SerialString.length()>0) 
    {
